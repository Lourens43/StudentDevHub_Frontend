...
export default function GenericTrack() {
  const { slug } = useParams();
  const { user } = useUser();
  const isAdmin = user?.role === "admin";

  const [trackMeta] = useState({
    title: slug,
    description: "Custom learning track",
    type: "Frontend",
  });
    ...
    <Layout>
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h1 className="text-3xl font-bold">{trackMeta.title}</h1>
            <p className="text-gray-600">{trackMeta.description}</p>
          </div>
        </div>
                ...
                {openIds.includes(module.id) && (
                  <>
                    <AdminResourcesSection
                      storageKey={`track_${slug}_module_${module.id}`}
                      isAdmin={isAdmin}
                      variant="rich"
                    />
                    <ModuleSubmissions
